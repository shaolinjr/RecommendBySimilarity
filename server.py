...
def get_rankings(user_id): #ta tendo alguma confusao de dados, usarios nao estao com os dados corretos
    ...
    df = list(r.db('usuarios').table('raw_data').order_by(index='user_id').filter(lambda user:user["user_id"] != int(user_id)).run())


    df_raw = pd.read_json(json.dumps(df))

    df_raw = df_raw.reindex_axis(['user_id', 'tipo_user', 'habilidades', 'segmento', 'horas_disponiveis','id'],
                                         axis=1)
    # print("\nRAW: ", df_raw)
    user   = pd.read_json(json.dumps(user))
    print("User before: ", user)
    user   = user.reindex_axis(['user_id', 'tipo_user', 'habilidades', 'segmento', 'horas_disponiveis','id'],
                                         axis=1)
    ...
    # # print(df_json.head())
    #
    user     = preprocessThisDf(user, df_raw)
    # #
    df_json  = preprocessThisDf(df_json, df_json)
    # # # raise Exception('xyz')
    ...
    df_full = pd.read_json(json.dumps(df))
    df_full.reindex(df_full['user_id'])
    return generateFullJSON(pd.read_json(json.dumps(df)), filtered_data)
    # return render_template('usuario.html',user=df)

def preprocessThisDf(df, complete_df):
